...
then
	...
	innobackupex --user=root --password=$PWD --slave-info --compress --no-timestamp --databases ="sunseap_portal_v1 sunseap_dlc sta_billing sptel_dlc solarpvx_v2 solarpvx skylab_tiot skylab_staff skylab_sso skylab_portal skylab_noc skylab_csp sbtest radius_sensors radius racktables powerdnsadmin performance_schema operation_center mysql mwc_dlc microservices inventory_management innologix_2 innologix icingaweb2 grafana gable_dlc dlc_vault dlc_staging dlc_multi_tenancy 3rd_reporting az_cfssl az_vault cfssl decada_demo dlc_billing_portal dlc_demo dlc_marketplace information_schema performance_schema sys" /home/azureuser/azureblob/"$HOSTNAME"/full/"$BACKUP_DATE"/
	echo "Full backup was completed at `date`" >> $LOG
elif [ "$TODAY" = "Sunday" ]
then
	echo "Start to backup at `date`" >> $LOG
	mkdir -p /home/azureuser/azureblob/"$HOSTNAME"/incremental/"$BACKUP_DATE"
	innobackupex --user=root --password=$PWD --slave-info --compress --no-timestamp --databases="sunseap_portal_v1 sunseap_dlc sta_billing sptel_dlc solarpvx_v2 solarpvx skylab_tiot skylab_staff skylab_sso skylab_portal skylab_noc skylab_csp sbtest radius_sensors radius racktables powerdnsadmin performance_schema operation_center mysql mwc_dlc microservices inventory_management innologix_2 innologix icingaweb2 grafana gable_dlc dlc_vault dlc_staging dlc_multi_tenancy 3rd_reporting az_cfssl az_vault cfssl decada_demo dlc_billing_portal dlc_demo dlc_marketplace information_schema performance_schema sys" --incremental /home/azureuser/azureblob/"$HOSTNAME"/incremental/"$BACKUP_DATE" --incremental-basedir=/home/azureuser/azureblob/"$HOSTNAME"/full/"$YESTERDAY"/
	echo "Incremental backup was completed at `date`" >> $LOG
else
	echo "Start to backup at `date`" >> $LOG
	mkdir -p /home/azureuser/azureblob/"$HOSTNAME"/incremental/"$BACKUP_DATE"
	innobackupex --user=root --password=$PWD --slave-info --compress --no-timestamp --databases="sunseap_portal_v1 sunseap_dlc sta_billing sptel_dlc solarpvx_v2 solarpvx skylab_tiot skylab_staff skylab_sso skylab_portal skylab_noc skylab_csp sbtest radius_sensors radius racktables powerdnsadmin performance_schema operation_center mysql mwc_dlc microservices inventory_management innologix_2 innologix icingaweb2 grafana gable_dlc dlc_vault dlc_staging dlc_multi_tenancy 3rd_reporting az_cfssl az_vault cfssl decada_demo dlc_billing_portal dlc_demo dlc_marketplace information_schema performance_schema sys" --incremental /home/azureuser/azureblob/"$HOSTNAME"/incremental/"$BACKUP_DATE" --incremental-basedir=/home/azureuser/azureblob/"$HOSTNAME"/incremental/"$YESTERDAY"/
	echo "Incremental backup was completed at `date`" >> $LOG
fi
